...
namespace BattleManager
{
    ...
    {
        private const string jsonString = // very long string  of the entire Json Monster Manual
"""
[
  {
    "name": "Aboleth",
...
""";
        // source: https://gist.githubusercontent.com/tkfu/9819e4ac6d529e225e9fc58b358c3479/raw/d4df8804c25a662efc42936db60cfbc0a5b19db8/srd_5e_monsters.json

        public static void GenerateCreatureStatBlocks()
        {
            List<Object> creatureTemp = JsonSerializer.Deserialize<List<Object>>(jsonString);
            List<Character> creatures;

            foreach (Object o in creatureTemp)
            {
                String creatureStr = o.ToString();
                creatureStr = creatureStr.Split("{")[1].Split("}")[0];
                MessageBox.Show(creatureStr);
                String[] creatureStrArr = creatureStr.Split("\n"); // breaks it down to ("name": "value")

                foreach (String s in creatureStrArr)
                {
                    if (s.Length <= 1) continue;
                    MessageBox.Show(s);
                    String name = s.Split(":")[0].Trim(); // "name"
                    name = name.Substring(1, name.Length - 2); // name
                    String value = s.Split(":")[1].Trim(); // "value",
                    value = value.Substring(1, value.Length - 3); // value
